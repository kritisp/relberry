          ...
          <Reveal delay={100}>
            <div className="py-32 border border-white/10 bg-[#111] rounded-lg text-center shadow-2xl">
              ...
              <button 
                ...
                className="bg-[#D4AF37] text-[#0F0F0F] px-8 py-4 font-black uppercase tracking-[0.2em] text-xs hover:bg-white transition-all duration-300 rounded-sm shadow-[0_0_20px_rgba(212,175,55,0.3)] hover:scale-105"
              >
                Continue Shopping
              </button>
            </div>
                  ...
                  <Reveal key={`${item.id}-${item.size}`} delay={i * 100}>
                    <div className="py-8 flex flex-col md:grid md:grid-cols-12 gap-6 items-center relative group">
                      <button 
                        ...
                        className="absolute top-8 right-0 text-white/30 hover:text-[#D4AF37] md:hidden transition-colors"
                      >
                        <X size={20} />
                      </button>

                      <div className="col-span-6 flex items-center w-full">
                        <div className="w-24 h-32 flex-shrink-0 bg-[#111] rounded-sm overflow-hidden mr-6 border border-white/5 group-hover:border-[#D4AF37]/30 transition-colors">
                           <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                        </div>
                        <div>
                          <h3 className="text-white font-bold text-base md:text-lg uppercase tracking-wide leading-tight mb-2 pr-8 md:pr-0 group-hover:text-[#D4AF37] transition-colors">{item.name}</h3>
                      ...
                      <div className="col-span-3 w-full md:w-auto flex justify-between md:justify-center items-center mt-2 md:mt-0">
                        <div className="flex items-center border border-white/20 bg-[#111] rounded-sm">
                          <button onClick={() => updateQuantity(item.id, item.size, item.quantity - 1)} className="w-10 h-10 flex items-center justify-center text-white/60 hover:text-[#D4AF37] transition-colors"><Minus size={14} /></button>
                          <div className="w-10 h-10 flex items-center justify-center text-white font-black text-sm">{item.quantity}</div>
                          <button onClick={() => updateQuantity(item.id, item.size, item.quantity + 1)} className="w-10 h-10 flex items-center justify-center text-white/60 hover:text-[#D4AF37] transition-colors"><Plus size={14} /></button>
                        </div>
                      </div>
                      ...
                      <div className="col-span-3 text-right hidden md:flex flex-col items-end justify-center">
                        ...
                        <button 
                          onClick={() => removeFromCart(item.id, item.size)}
                          className="text-white/30 hover:text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.2em] transition-colors flex items-center border-b border-transparent hover:border-[#D4AF37]"
                        >
              ...
              <Reveal delay={200} direction="left">
                <div className="bg-[#111] p-8 md:p-10 rounded-lg lg:sticky top-32 border border-white/10 shadow-2xl">
                  ...
                  <div className="space-y-5 mb-8 text-sm font-bold uppercase tracking-wider">
                    ...
                    {shipping > 0 && (
                      <div className="bg-[#D4AF37]/10 border border-[#D4AF37]/20 p-3 rounded-sm">
                         <p className="text-[#D4AF37] text-[10px] tracking-widest uppercase text-center">Add ₹{2000 - subtotal} more for free shipping</p>
                      </div>
                    )}
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full bg-[#D4AF37] text-[#0F0F0F] py-5 font-black uppercase tracking-[0.2em] text-xs hover:bg-white transition-all duration-300 rounded-sm shadow-[0_0_20px_rgba(212,175,55,0.3)] hover:shadow-[0_0_40px_rgba(255,255,255,0.5)] active:scale-95 hover:-translate-y-1"
                  >
                    Proceed to Checkout
                  </button>
                </div>
