    ...
    <div className="w-full bg-[#0F0F0F] min-h-screen pt-24 pb-32">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <Reveal direction="down">
          <button 
            ...
            className="flex items-center text-white/50 hover:text-gold-metallic mb-8 transition-colors text-xs font-bold uppercase tracking-[0.1em] group"
          >
                ...
                <div className="w-full aspect-[4/5] bg-[#111] overflow-hidden rounded-lg relative border border-white/5 group cursor-zoom-in">
                   {idx === 0 && (
                     <div className="absolute top-6 left-6 z-10 flex flex-col gap-3">
                       {product.tag && (
                        <div className="bg-gold-metallic text-[#0F0F0F] text-[10px] font-black uppercase tracking-[0.2em] px-4 py-2 shadow-[0_0_15px_rgba(255,214,78,0.3)]">
                          {product.tag}
                        </div>
                       )}
                       {discount > 0 && (
          ...
          <div className="w-full lg:w-[45%]">
            <div className="lg:sticky top-32 py-4">
              <Reveal delay={100} direction="left">
                <p className="text-gold-metallic text-xs font-black uppercase tracking-[0.2em] mb-4 drop-shadow-[0_0_10px_rgba(255,214,78,0.3)]">{product.category}</p>
              ...
              <Reveal delay={200} direction="left">
                <div className="mb-10">
                  <div className="flex justify-between items-end mb-4">
                    ...
                    <button className="text-white/40 hover:text-gold-metallic text-xs font-bold underline tracking-wider transition-colors">Size Guide</button>
                  </div>
                    ...
                    {['S', 'M', 'L', 'XL'].map(size => (
                      <button 
                        ...
                        className={`relative py-4 text-sm font-black uppercase tracking-wider transition-all duration-300 rounded-sm border ${
                          selectedSize === size 
                            ? 'border-gold-metallic bg-gold-metallic/10 text-gold-metallic shadow-[0_0_15px_rgba(255,214,78,0.2)] scale-105' 
                ...
                <div className="mb-12">
                  ...
                  <div className="flex items-center w-32 border border-white/20 bg-[#111] rounded-sm">
                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-12 flex items-center justify-center text-white/60 hover:text-gold-metallic transition-colors"><Minus size={16} /></button>
                    <div className="flex-1 h-12 flex items-center justify-center text-white font-black text-sm">{quantity}</div>
                    <button onClick={() => setQuantity(quantity + 1)} className="w-10 h-12 flex items-center justify-center text-white/60 hover:text-gold-metallic transition-colors"><Plus size={16} /></button>
                  </div>
                </div>
              </Reveal>
              ...
              <Reveal delay={300} direction="left">
                <div className="flex flex-col gap-4 hidden md:flex">
                  <button 
                    ...
                    className="w-full border border-gold-metallic bg-gold-metallic/5 text-gold-metallic py-5 font-black uppercase tracking-[0.2em] text-xs hover:bg-gold-metallic hover:text-[#0F0F0F] transition-all duration-300 rounded-sm hover:shadow-[0_0_30px_rgba(255,214,78,0.4)] active:scale-[0.98]"
                  >
                    Add to Cart
                  </button>
                  <button 
              ...
              <Reveal delay={400} direction="left">
                <div className="mt-12 space-y-5 pt-8 border-t border-white/10 bg-[#111] p-6 rounded-lg border-l-2 border-l-[#d4af37]">
                  <div className="flex items-center text-white/70 text-xs font-bold uppercase tracking-widest">
                    <span className="mr-4 text-gold-metallic">🚚</span> Free shipping over ₹2000
                  </div>
                  <div className="flex items-center text-white/70 text-xs font-bold uppercase tracking-widest">
                    <span className="mr-4 text-gold-metallic">🛡️</span> High-Density Print Guarantee
                  </div>
                  <div className="flex items-center text-white/70 text-xs font-bold uppercase tracking-widest">
                    <span className="mr-4 text-gold-metallic">↻</span> 7-Day hassle-free returns
                  </div>
                </div>
              </Reveal>
            </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-[#0F0F0F]/90 backdrop-blur-xl border-t border-white/10 p-4 z-50 animate-fade-in-up">
         <div className="flex gap-3">
            <button 
              onClick={handleAddToCart}
              className="flex-1 border border-gold-metallic text-gold-metallic py-4 font-black uppercase tracking-wider text-xs rounded-sm active:bg-gold-metallic/20"
            >
              Add to Cart
            </button>
            <button 
              onClick={() => { handleAddToCart(); navigate('checkout'); }}
              className="flex-1 bg-gold-metallic text-[#0F0F0F] py-4 font-black uppercase tracking-wider text-xs rounded-sm shadow-[0_0_15px_rgba(255,214,78,0.3)] active:scale-[0.98]"
            >
              Buy Now
            </button>
         </div>
