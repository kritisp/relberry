...
const ProductCard = ({ product, navigate, onQuickAdd }) => {
  ...
  return (
    <div className="group flex flex-col cursor-pointer bg-transparent rounded-lg overflow-hidden transition-all duration-500 hover:-translate-y-2">
      <div 
        className="relative aspect-[3/4] overflow-hidden bg-[#111] rounded-lg border border-white/5 group-hover:border-gold-metallic/30 group-hover:shadow-[0_10px_40px_rgba(255,214,78,0.1)] transition-all duration-500"
      ...
      >
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 z-20">
          ...
          {discount > 0 && (
            <div className="bg-gold-metallic text-[#0F0F0F] text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-sm shadow-[0_0_15px_rgba(255,214,78,0.4)] w-max animate-pulse-slow">
              {discount}% OFF
            </div>
          )}
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 translate-y-[120%] group-hover:translate-y-0 transition-transform duration-300 ease-out z-20">
          <button 
            ...
            className="w-full bg-gold-metallic text-[#0F0F0F] py-3.5 font-black uppercase tracking-wider text-xs rounded-sm hover:bg-white hover:shadow-[0_0_20px_rgba(255,255,255,0.5)] transition-all duration-300 flex items-center justify-center gap-2 active:scale-95"
          >
